...
class TestPyircor(unittest.TestCase):
    ...
    def test_tauap_b(self):
        ...
        self.assertAlmostEqual(tauap_b(set1['x'], set1['y']), 0.8333, delta=5e-5)
        self.assertAlmostEqual(tauap_b(set2['x'], set2['y']), 0.2880, delta=5e-5)
        self.assertAlmostEqual(tauap_b(set3['x'], set3['y']), -.668, delta=5e-5)

        # now with ties
        self.assertAlmostEqual(tauap_b(set1['x_ties'], set1['y_ties']), 0.7321, delta=5e-5)
        self.assertAlmostEqual(tauap_b(set2['x_ties'], set2['y_ties']), 0.297, delta=5e-5)
        self.assertAlmostEqual(tauap_b(set3['x_ties'], set3['y_ties']), -.7047, delta=5e-5)
        # and symmetry
        self.assertAlmostEqual(tauap_b(set1['y_ties'], set1['x_ties']), 0.7321, delta=5e-5)
        self.assertAlmostEqual(tauap_b(set2['y_ties'], set2['x_ties']), 0.297, delta=5e-5)
        self.assertAlmostEqual(tauap_b(set3['y_ties'], set3['x_ties']), -.7047, delta=5e-5)
